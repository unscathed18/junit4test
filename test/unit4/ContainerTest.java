...
public class ContainerTest {
    ...
    public void testRemove() {
        container.clear();
        assertFalse("container.remove(Package) should fail because Container is empt, but it didn't", container.remove(b[0]));
        for(int i = 0; i < num_Packages_To_Test; i++){
            container.clear();
            for(int j =0; j < 1; j++){
                container.add(b[j]);
    ...
    public void testGetVolume() {
        ...
        container.clear();
        for(int i =0; i < this.num_Packages_To_Test; i++){
            container.add(b[i]);
        }
